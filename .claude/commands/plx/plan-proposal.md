...
**Steps**
0. Check for existing request context: If `$ARGUMENTS` specifies a change-id, check for `workspace/changes/<id>/request.md`. If it exists, read it and use the **Final Intent** section as the primary input for this proposal. Incorporate decisions from the **Decisions** section into the proposal context. If no request.md exists or no change-id is provided, proceed normally.
...
2. Choose a unique verb-led `change-id` (or use the existing one from Step 0) and scaffold `proposal.md`, `tasks/` directory, and `design.md` (when needed) under `workspace/changes/<id>/`.
...
**Reference**
- Run `plx/plan-request` first to clarify intent and create `request.md` before running this command.
