...
- **THEN** check if id exists in `openspec/changes/` → archive as change
- **AND** check if id exists in `openspec/reviews/` → archive as review
...
- **THEN** look only in `openspec/reviews/` for the id
