...
**Goals:**
- PLX slash commands installed during `plx init` alongside PLX commands
- Architecture documentation generated by AI agents via slash commands
- Clean separation between PLX and PLX functionality
...
- Modifying PLX's core command structure
...
**Decision:** PLX commands use the same `<!-- PLX:START -->` / `<!-- PLX:END -->` marker pattern.

**Rationale:** Consistency with existing marker system, allows `plx update` to refresh PLX content if needed in the future.
...
1. Existing PLX installations continue to work unchanged
