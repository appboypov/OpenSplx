...
The GitHub repository is renamed from `OpenSplx` to `pew-pew-splx`, with automatic redirects from the old URL.

## Currently

The repository is at `github.com/appboypov/OpenSplx`. All documentation references the new name `pew-pew-splx` but the actual repository URL uses the old name.

## Should

- Repository URL is `github.com/appboypov/pew-pew-splx`
...
- [ ] `https://github.com/appboypov/pew-pew-splx` resolves to repository
...
- [x] Change name from `OpenSplx` to `pew-pew-splx`
- [x] Click "Rename" button
- [x] Confirm in modal if prompted
- [x] Verify new URL works
- [x] Verify old URL redirects
- [x] Optionally update local remote: `git remote set-url origin https://github.com/appboypov/pew-pew-splx.git`
