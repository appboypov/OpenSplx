...
## Constraints
- [ ] Must preserve existing status transition behavior
- [ ] Must handle Implementation Checklist checkbox marking
- [ ] Must work with multi-workspace task discovery

## Acceptance Criteria
- [ ] `plx complete task --id <id>` finds and completes task in centralized storage
- [ ] `plx complete change --id <id>` finds all tasks with parent-id=<id> and completes them
- [ ] `plx undo task --id <id>` reverts task in centralized storage
- [ ] `plx undo change --id <id>` reverts all tasks linked to change

## Implementation Checklist
- [ ] 4.1 Update `src/commands/complete.ts`:
