...
## Purpose
Provide shell completion scripts for the PLX CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) in supported shells. Currently supports Zsh with architecture designed for future shell expansion.
## Requirements
...
- **WHEN** user executes `plx completion generate zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_plx` if Oh My Zsh is detected
...
- **AND** allow redirection to files: `plx completion generate zsh > /path/to/_plx`
