...
The CLI SHALL provide a `splx transfer spec` command that moves a spec directory along with related changes and their linked tasks.
...
- **WHEN** user runs `splx transfer spec --id user-auth --target /path/to/target`
...
- **WHEN** user runs `splx transfer spec --id config --target /path/to/target`
...
- **WHEN** user runs `splx transfer task --id 002-add-feature-implement --target /path/to/target --target-name new-task-name`
...
The CLI SHALL provide a `splx transfer review` command that moves a review directory and its linked tasks.

#### Scenario: Transfer review with linked tasks
- **GIVEN** a review `code-review-1` exists with 2 linked tasks
- **WHEN** user runs `splx transfer review --id code-review-1 --target /path/to/target`
...
The CLI SHALL provide a `splx transfer request` command that moves a request.md file from a change directory.

#### Scenario: Transfer request
- **GIVEN** a change `add-feature` exists with a `request.md` file
- **WHEN** user runs `splx transfer request --id add-feature --target /path/to/target`
...
- **WHEN** user runs `splx transfer change --id X --target /path/to/target --yes`
...
- **WHEN** user runs `splx transfer change --id add-feature --target /path/to/target --dry-run`
...
- **WHEN** user runs `splx transfer change --id X --target /path/to/target --dry-run`
...
- **WHEN** user runs `splx transfer change --id add-feature` without `--target`
...
- **WHEN** user runs `splx transfer change --id X --no-interactive`
