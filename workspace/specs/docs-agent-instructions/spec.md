...
`workspace/AGENTS.md` SHALL include complete copy/paste templates and inline examples exactly where agents make corresponding edits.
...
`workspace/AGENTS.md` SHALL offer a concise pre-validation checklist that highlights common formatting mistakes before running `splx validate`.
...
- **WHEN** reorganizing `workspace/AGENTS.md`
...
`workspace/AGENTS.md` and slash command templates SHALL instruct AI assistants to use their available question tools for gathering clarifications instead of asking questions directly in chat.
...
- **WHEN** an AI assistant reads the "Before Creating Specs" section in `workspace/AGENTS.md`
